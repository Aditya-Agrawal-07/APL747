...
"""
Response Generation: Lotka-Volterra model:
"""
def lotka_volterra(x0, tparam, sysparam):
    # Statespace of the Lotka-Volterra system:
    def fun(y,t,params):
        a, b, c, d = params
        dydt = np.array([(a*y[0] - b*y[0]*y[1]), 
                         (d*y[0]*y[1] - c*y[1])])
        return dydt
    ...
    # Time integration:
    xt = odeint(fun, x0, t_eval, args=(sysparam,))    
    return xt.T, t_eval

"""
Response Generation: Burgers equation:
"""
def pde_oned_Galerkin(u0,T,a,N,J,epsilon):
    Dx = a/J
    ...
    for n in range(N):           # time loop
        fhu = fft(fun_handle(u))    # evaluate fhat(u)
